...
if 'datapath' not in rcParams:
    ...
    def get_path(image_file):
        path = os.path.join(rcParams['datapath'], 'images', image_file + ".png")
        if not os.path.exists(path):
            path = os.path.join(rcParams['datapath'], 'images', image_file + ".gif")
            if not os.path.exists(path):
                path = ""
        return path
...
class NavigationToolbar(NavigationToolbar2Tk):
    toolitems = (
        ('Home', _('Reset original view'), get_path('home'), 'home'),
        ('Back', _('Back to previous view'), get_path('back'), 'back'),
        ('Forward', _('Forward to next view'), get_path('forward'), 'forward'),
        (None, None, None, None),
        ...
        (None, None, None, None),
        ('Save', _('Save the figure'), get_path('filesave'), 'save_figure'),
    )
